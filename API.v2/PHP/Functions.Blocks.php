			...
			switch ($Key) {
				case 'filename':
					if (!empty($Data['image']['isStaticImage'])) {
						...
						$Image    = getApiData($getHashParams);
...
function Listing($Data, $ItemFormat, $ChildFormat = '', $TagsType = 'Div', $Child = '') {
	...
	if ($TagsType == 'Table') $Tags = array('Strings' => array('Opening' => "<td>", 'Closing' => '</td>'), 'Return' => array('Opening' => "<tr " . (!empty($Data['values'][0]['callout']) ? "class='Active TR-{$Data['values'][0]['callout']}' data-ID='{$Data['values'][0]['callout']}'" : "") . ">", 'Closing' => '</tr>'));
	...
	if ($vinSearchParameters['examples']) {
		...
		if (!empty($_GET['vin'])) {
			...
			if ($VinData['data']['vins']) {
				...
				$Vins = "<div class='VinInfo " . (!empty($CurrentVin) ? 'Hidden' : "") . "'>" . ImplodeIfArray($Vins) . "</div>";
...
function ifButtonsSet($Catalogs) {
	...
	return ImplodeIfArray($CatalogGroup);
}
