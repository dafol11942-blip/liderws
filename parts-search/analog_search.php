...
if (!function_exists('getAjaxFactory')) {
...
if (!function_exists('fmtDeliveryHtml')) {
...
if (!function_exists('calcDelivery')) {
...
if (!function_exists('maskWarehouse')) {
...
try {
    ...
    // Режим final: P2 завершён, читаем из state-файла
    if ($phase === 'final' && !empty($p2Hash)) {
        ...
        $p2Hash = ''; // сбрасываем, чтобы не запускать P2 повторно
    } else {
        if ($phase === 'fast') {
        ...
        // Дозагрузка кросс-номеров из MySQL
        $allBrandNorms = [];
        ...
        if (!empty($allBrandNorms)) {
            ...
            while ($row = $res->fetch_assoc()) {
                ...
                if (!isset($crossArticles[$na])) continue;
