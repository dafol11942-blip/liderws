...
async function loadResults(){
    ...
    // а не сразу после фазы 1: это второстепенные данные, которые могут подождать, а фаза 2
    // (докрутка складов у поставщиков) — критичный путь. Раньше запрос уходил параллельно
    // с докруткой и, будучи медленным на холодном кэше (десятки аналогов сразу), заметно
    // задерживал именно её — прогресс докрутки зависал на старом значении на ~30 секунд.
    loadAnalogCrossInfo(d1.analogs || []);
}
...
// Карточки UMAPI для аналогов — та же идея, что и loadCrossInfo() для искомого номера, но
// одним батч-запросом сразу на все группы аналогов. Вызывается из loadResults() ПОСЛЕ фазы 2
// (докрутки складов), а не параллельно с ней — это второстепенные данные, которые не должны
// соревноваться за ресурсы хостинга с критичной докруткой (см. историю инцидента с
// перегрузкой внешних API в STAGES.md). analogCrossRequested не даёт переспросить уже
// запрошенные ключи при последующих renderResults() (смена фильтра и т.п.). Не блокирует и
// не задерживает страницу: если UMAPI медленная/недоступна, у аналогов просто не появляются
// блоки.
