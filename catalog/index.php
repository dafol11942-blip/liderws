...
// Товары с нулевым остатком не показываем нигде в каталоге. В разделах
// фильтруем на уровне SQL (до пагинации компонента), иначе она режет
// строго по PAGE_ELEMENT_COUNT ДО того, как узнаёт, что часть уже
// отобранных товаров пуста, и страница остаётся недобитой. На корне
// каталога ("Все товары") компонент работает в особом режиме
// SHOW_ALL_WO_SECTION — с этим фильтром в $arrFilter он там ломается
// и не находит ничего, поэтому там полагаемся на построчную проверку
// в result_modifier.php (недобитые страницы там — приемлемый компромисс).
if ($sectionId > 0) {
