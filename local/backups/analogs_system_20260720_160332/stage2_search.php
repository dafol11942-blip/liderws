...
// ВАЖНО: не трогаем $batchReq/$batchSeen/$addToBatch (closure use-by-ref).
// Отдельные $enrReq/$enrMeta, иначе добор аналогов молча пустой.
$analogKeyCount = [];
$analogKeySamples = [];
...
foreach ($enrichKeys as $gk) {
    ...
    if (isset($brandmapMeta[$gk]) && is_array($brandmapMeta[$gk])) {
        ...
        foreach (($info['brands'] ?? []) as $src => $b) {
            $a = trim((string)($info['articles'][$src] ?? $info['article_nr'] ?? $baseArt));
            $b = trim((string)$b);
            if ($b === '' || $a === '') {
                continue;
            }
