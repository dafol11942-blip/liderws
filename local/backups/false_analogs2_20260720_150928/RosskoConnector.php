...
class RosskoConnector implements SupplierInterface
{
    ...
    public function parseSearchResponse(string $responseBody, string $brand, string $article): array
    {
        ...
        // Используем BrandNormalizer
        $normTargetBrand   = BrandNormalizer::normalize($brand);
        $normTargetArticle = BrandNormalizer::normalizeArticle($article);

        foreach ($pn as $part) {
            $pBrand   = trim((string)($part->xpath('*[local-name()="brand"]')[0] ?? ''));
            $pArticle = trim((string)($part->xpath('*[local-name()="partnumber"]')[0] ?? ''));

            if (BrandNormalizer::normalize($pBrand) !== $normTargetBrand) continue;
            if (BrandNormalizer::normalizeArticle($pArticle) !== $normTargetArticle) continue;

            $ds = $part->xpath('*[local-name()="stocks"]/*[local-name()="stock"]');
            if ($ds) { foreach ($ds as $s) $results[] = $this->parseStock($s, $part); }

            $cp = $part->xpath('*[local-name()="crosses"]/*[local-name()="Part"]');
            if ($cp) {
                foreach ($cp as $c) {
                    $cs = $c->xpath('*[local-name()="stocks"]/*[local-name()="stock"]');
                    if ($cs) { foreach ($cs as $s) $results[] = $this->parseStock($s, $c); }
                }
            }
        }
        ...
        foreach ($results as $item) {
            $key = !empty($item->stockId) ? $item->stockId : md5($item->warehouse . "|" . $item->price);
