...
// ---- ANALOG ENRICHMENT v3 ----
// После cross/exact: для топ-аналогов делаем exact-запросы ко ВСЕМ поставщикам.
// Иначе в карточке аналога остаётся только тот, кто пришёл из cross (часто ixora).
$analogKeyCount = [];
$analogKeySamples = [];
foreach ($allResults as $it) {
    ...
    if (isset($detectFamily, $targetFamily) && $targetFamily !== '') {
        $fam = $detectFamily((string)$it->name . ' ' . $it->brand . ' ' . $it->article);
        if ($fam !== '' && $fam !== $targetFamily) {
            continue;
        }
    }
    $analogKeyCount[$gk] = ($analogKeyCount[$gk] ?? 0) + 1;
    if (!isset($analogKeySamples[$gk])) {
        $analogKeySamples[$gk] = [
...
arsort($analogKeyCount);
$enrichKeys = array_slice(array_keys($analogKeyCount), 0, 15);

$enrReq = [];
$enrMeta = [];
$enrSeen = [];

foreach ($enrichKeys as $gk) {
    $sample = $analogKeySamples[$gk] ?? null;
    if (!$sample) {
        continue;
    }
    $baseBrand = trim((string)$sample['brand']);
    $baseArt = trim((string)$sample['article']);
    if ($baseBrand === '' || $baseArt === '') {
        continue;
    }

    // варианты brand/article: sample + brandmap per supplier
    $variantMap = [];
    $variantMap[mb_strtolower($baseBrand) . "\0" . mb_strtolower($baseArt)] = [$baseBrand, $baseArt];
    if (isset($brandmapMeta[$gk]) && is_array($brandmapMeta[$gk])) {
        $info = $brandmapMeta[$gk];
        foreach (($info['brands'] ?? []) as $src => $b) {
            $a = (string)($info['articles'][$src] ?? $info['article_nr'] ?? $baseArt);
            $b = trim((string)$b);
            $a = trim($a);
            if ($b === '' || $a === '') continue;
            if (BrandNormalizer::groupKey($b, $a) !== $gk) continue;
            $variantMap[mb_strtolower($b) . "\0" . mb_strtolower($a)] = [$b, $a];
        }
    }
    ...
    foreach (getSupplierFactory()->allAvailable() as $sup) {
        $code = $sup->getCode();
        $tries = [];
        // 1) написание этого поставщика из brandmap
        if (isset($brandmapMeta[$gk]['brands'][$code], $brandmapMeta[$gk]['articles'][$code])) {
            $tries[] = [
                trim((string)$brandmapMeta[$gk]['brands'][$code]),
                trim((string)$brandmapMeta[$gk]['articles'][$code]),
            ];
        }
        // 2) sample + ещё один variant
        $n = 0;
        foreach ($variantMap as $va) {
            $tries[] = $va;
            $n++;
            if ($n >= 2) break;
        }
        ...
        foreach ($tries as $va) {
            ...
            if ($b === '' || $a === '') continue;
            if (BrandNormalizer::groupKey($b, $a) !== $gk) continue;
            $uniqTry[mb_strtolower($b) . "\0" . mb_strtolower($a)] = [$b, $a];
        }

        foreach ($uniqTry as $va) {
            $b = $va[0];
            $a = $va[1];
            $dedupe = $code . "\0" . mb_strtolower($b) . "\0" . mb_strtolower($a);
            if (isset($enrSeen[$dedupe])) {
                continue;
            }
            $enrSeen[$dedupe] = true;

            $ck = md5('enr3|' . $code . '|' . BrandNormalizer::normalize($b) . '|' . BrandNormalizer::normalizeArticle($a) . '|' . mb_strtolower($b) . '|' . mb_strtolower($a));
            $cached = $itemCache->get($ck);
...
if ($enrReq) {
    $chunksR = array_chunk($enrReq, 20, true);
    $chunksM = array_chunk($enrMeta, 20, true);
    foreach ($chunksR as $ci => $chunk) {
        $mh = curl_multi_init();
        $hs = [];
        foreach ($chunk as $i => $r) {
        ...
        foreach ($hs as $i => $ch) {
            ...
            if ($http === 200 && $body) {
                try {
                    ...
                    unset($body);
                    $wantGk = $m['gk'] ?? BrandNormalizer::groupKey($m['brand'], $m['article']);
                    $gc = 0;
                    foreach ($items as $it) {
