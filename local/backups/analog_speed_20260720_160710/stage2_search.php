...
// =============================================================================
// ANALOG FILL v1 (systemic)
// -----------------------------------------------------------------------------
// 1) Собираем кандидатов-аналогов из уже полученных results + brandmap
// 2) Для каждого кандидата делаем EXACT-запрос ко ВСЕМ поставщикам
// 3) Фильтры: не омонимы артикула, family mismatch, caps по памяти
// НЕ трогаем $batchReq/$batchSeen/$addToBatch (use-by-ref closures).
// =============================================================================
...
// family целиком из exact/display, если ещё не задана
if (!isset($targetFamily) || $targetFamily === '') {
    ...
    // fallback: имя exact-позиций из results
    if ($targetFamily === '') {
...
$analogCandidates = []; // gk => ['brand'=>, 'article'=>, 'name'=>, 'score'=>]

$considerAnalog = static function (string $brand, string $article, string $name = '', int $score = 1) use (
    &$analogCandidates, $exactKey, $normTargetBrand, $normTargetArt, $detectFamily, $targetFamily
) {
    $brand = trim($brand);
    $article = trim($article);
    if ($brand === '' || $article === '') {
    ...
    $gk = BrandNormalizer::groupKey($brand, $article);
    if ($gk === '' || $gk === $exactKey) {
        return;
    }
    // омоним: тот же артикул, другой бренд — НЕ аналог
    if (BrandNormalizer::normalizeArticle($article) === $normTargetArt
        && BrandNormalizer::normalize($brand) !== $normTargetBrand) {
        return;
    }
    if ($targetFamily !== '') {
        $fam = $detectFamily($name . ' ' . $brand . ' ' . $article);
        if ($fam !== '' && $fam !== $targetFamily) {
            // pad/brake совместимы
            $padLike = ['pad', 'brake'];
    ...
    if ($name !== '' && mb_strlen($name) > mb_strlen((string)$analogCandidates[$gk]['name'])) {
        $analogCandidates[$gk]['name'] = $name;
    }
    // предпочитаем более «красивое» написание артикула/бренда
    if (mb_strlen($brand) >= 2) {
        $analogCandidates[$gk]['brand'] = BrandNormalizer::displayBrand($brand) ?: $brand;
    }
    if ($article !== '') {
        $analogCandidates[$gk]['article'] = $article;
    }
};

// из результатов поиска (cross/exact/extra)
foreach ($allResults as $it) {
    if (!($it instanceof SearchResultItem)) {
        continue;
    }
    $considerAnalog($it->brand, $it->article, (string)$it->name, 3);
}

// из brandmap (другие артикулы; не омонимы)
if (!empty($cachedBrandMap) && is_array($cachedBrandMap)) {
    foreach ($cachedBrandMap as $mk => $info) {
        if ($mk === $exactKey || $mk === ($targetKey ?? '')) {
            continue;
        }
        ...
        $considerAnalog($bDisp, $aDisp, $desc, $score);
    }
}

// сортировка кандидатов: score desc
uasort($analogCandidates, static function ($a, $b) {
    return ($b['score'] ?? 0) <=> ($a['score'] ?? 0);
});

// лимит кандидатов (память/время). 18 — баланс полноты и скорости
$analogCandidates = array_slice($analogCandidates, 0, 18, true);

// ---- multi-supplier EXACT fill for each analog candidate ----
$fillReq = [];
$fillMeta = [];
$fillSeen = [];

foreach ($analogCandidates as $gk => $cand) {
    $baseBrand = (string)$cand['brand'];
    $baseArt = (string)$cand['article'];

    foreach (getSupplierFactory()->allAvailable() as $sup) {
        $code = $sup->getCode();

        // варианты написания для поставщика
        $tries = [];
        if (isset($brandmapMeta[$gk]['brands'][$code], $brandmapMeta[$gk]['articles'][$code])) {
            $tries[] = [
                (string)$brandmapMeta[$gk]['brands'][$code],
                (string)$brandmapMeta[$gk]['articles'][$code],
            ];
        }
        $tries[] = [$baseBrand, $baseArt];
        // displayBrand variant
        $tries[] = [BrandNormalizer::displayBrand($baseBrand), $baseArt];
        ...
        foreach ($tries as $va) {
            ...
            $a = trim((string)$va[1]);
            if ($b === '' || $a === '') continue;
            if (BrandNormalizer::groupKey($b, $a) !== $gk) continue;
            $uniq[mb_strtolower($b) . "\0" . mb_strtolower($a)] = [$b, $a];
        }

        // максимум 2 написания на поставщика
        $nTry = 0;
        foreach ($uniq as $va) {
            $b = $va[0];
            $a = $va[1];
            $dedupe = $code . "\0" . mb_strtolower($b) . "\0" . mb_strtolower($a) . "\0" . $gk;
            if (isset($fillSeen[$dedupe])) {
                continue;
            }
            $fillSeen[$dedupe] = true;

            $ck = md5('afill1|' . $code . '|' . $gk . '|' . mb_strtolower($b) . '|' . mb_strtolower($a));
            $cached = $itemCache->get($ck);
            if (is_array($cached) && !empty($cached['ok']) && !empty($cached['rows']) && is_array($cached['rows'])) {
                ...
                // если уже есть кеш — второе написание не нужно
                break;
            }
            ...
            $nTry++;
            if ($nTry >= 1) {
                // одного точного написания достаточно; brandmap-first уже учтён
                break;
            }
        }
    }
...
// execute fill requests
if ($fillReq) {
    $chunksR = array_chunk($fillReq, 20, true);
    $chunksM = array_chunk($fillMeta, 20, true);
    foreach ($chunksR as $ci => $chunk) {
        $mh = curl_multi_init();
        $hs = [];
        foreach ($chunk as $i => $r) {
        ...
        foreach ($hs as $i => $ch) {
            ...
            if ($http === 200 && $body) {
                try {
                    ...
                    foreach ($items as $it) {
                        ...
                        if ($igk !== $wantGk) continue;

                        // family safety on fill too
                        if ($targetFamily !== '') {
            ...
            // кешируем только непустые — пустой ответ не блокирует другие написания/повторы надолго
            if ($rows) {
                $itemCache->set($m['ck'], ['ok' => 1, 'rows' => $rows], 600);
            }
        }
