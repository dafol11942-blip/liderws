...
class BergConnector implements SupplierInterface
{
    ...
    public function buildSearchRequest(string $brand, string $article): ?array
    {
        if (!$this->isAvailable()) return null;
        $url = rtrim($this->baseUrl, '/') . '/ordering/get_stock.json';
        $body = ['items' => [['resource_article' => $article, 'brand_name' => $brand]]];
        if ($this->addressId) $body['address_id'] = $this->addressId;
        $json = json_encode($body);
        return [
            'url'     => $url,
    ...
    public function parseSearchResponse(string $responseBody, string $brand, string $article): array
    {
        ...
        foreach ($data['resources'] as $res) {
            $resBrand = $res['brand']['name'] ?? '';

            // Используем BrandNormalizer вместо дубликата mapBrand
            if (BrandNormalizer::normalize($resBrand) !== $normTargetBrand) continue;
