...
// ---- ANALOG ENRICHMENT ----
// Кроссы дают brand|article в основном от partkom/ixora.
// Добираем exact-предложения этих аналогов у ВСЕХ поставщиков (msk/rossko/berg...),
// иначе в аналогах "везде Иксора".
$analogKeyCount = [];
foreach ($allResults as $it) {
    if (!$it instanceof SearchResultItem) continue;
    $gk = BrandNormalizer::groupKey($it->brand, $it->article);
    if ($gk === $exactKey) continue;
    // skip homonyms same article other brand
    if (BrandNormalizer::normalizeArticle($it->article) === $normTargetArt
        && BrandNormalizer::normalize($it->brand) !== $normTargetBrand) {
        continue;
    }
...
$enrichKeys = array_slice(array_keys($analogKeyCount), 0, 12);
$batchReq = [];
$batchMeta = [];
$batchSeen = [];
foreach ($enrichKeys as $gk) {
    [$eb, $ea] = array_pad(explode('|', $gk, 2), 2, '');
    // need display brand/article from any item
    $eBrand = '';
    $eArt = '';
    foreach ($allResults as $it) {
        if (BrandNormalizer::groupKey($it->brand, $it->article) === $gk) {
            $eBrand = $it->brand;
            $eArt = $it->article;
            break;
        }
    }
    if ($eBrand === '' || $eArt === '') continue;
    // family guard
    if (isset($detectFamily, $targetFamily) && $targetFamily !== '') {
        $ok = false;
        foreach ($allResults as $it) {
            if (BrandNormalizer::groupKey($it->brand, $it->article) !== $gk) continue;
            $fam = $detectFamily((string)$it->name . ' ' . $it->brand);
            if ($fam === '' || $fam === $targetFamily) { $ok = true; break; }
        }
        if (!$ok) continue;
    }
    foreach (getSupplierFactory()->allAvailable() as $sup) {
        $addToBatch($sup, $eBrand, $eArt, false, 5);
    }
}
if ($batchReq) {
    $chunksR = array_chunk($batchReq, 25, true);
    ...
    foreach ($chunksR as $ci => $chunk) {
        ...
        foreach ($chunk as $i => $r) {
            ...
            if (($r['method'] ?? 'GET') === 'POST') {
                curl_setopt($ch, CURLOPT_POST, true);
                if (!empty($r['body'])) curl_setopt($ch, CURLOPT_POSTFIELDS, $r['body']);
            }
            curl_multi_add_handle($mh, $ch);
            $hs[$i] = $ch;
        }
        $rn = null;
        do { curl_multi_exec($mh, $rn); curl_multi_select($mh, 0.05); } while ($rn > 0);
        foreach ($hs as $i => $ch) {
            $body = curl_multi_getcontent($ch);
            $http = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
            curl_multi_remove_handle($mh, $ch);
            ...
            if ($http === 200 && $body) {
                try {
                    $items = $m['sup']->parseSearchResponse($body, $m['brand'], $m['article']);
                    if (is_array($items) && count($items) > 80) {
                    ...
                    foreach ($items as $it) {
                        ...
                        // only keep this analog key / exact
                        $gk = BrandNormalizer::groupKey($it->brand, $it->article);
                        if ($gk !== BrandNormalizer::groupKey($m['brand'], $m['article']) && $gk !== $exactKey) {
                            continue;
                        }
                        $groupCount[$gk] = ($groupCount[$gk] ?? 0) + 1;
                        if ($groupCount[$gk] > 8) continue;
                        $rows[] = $itemToArray($it);
                        $allResults[] = $it;
                    }
                } catch (\Throwable $e) {}
