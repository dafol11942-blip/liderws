...
$fullCache = new \Lider\Search\SearchCacheManager('/search/s2_full_v26', 300);
...
$itemCache = new \Lider\Search\SearchCacheManager('/search/s2_items_v26', 600);
$allResults = [];
$batchReq = [];
$batchMeta = [];
$batchSeen = [];
...
try {
    if (!isset($targetFamily) || $targetFamily === '') {
        ...
        if (!empty($targetEntry['description'])) {
            $targetFamily = \Lider\Search\AnalogOfferFiller::detectFamily((string)$targetEntry['description']);
        }
        if ($targetFamily === '' && !empty($displayBrand)) {
            $targetFamily = \Lider\Search\AnalogOfferFiller::detectFamily((string)$displayBrand . ' ' . (string)$displayArticle);
        }
        if ($targetFamily === '') {
            foreach ($allResults as $__it) {
                if ($__it instanceof \Lider\Search\SearchResultItem
                    && \Lider\Search\BrandNormalizer::groupKey($__it->brand, $__it->article) === $exactKey) {
                    $targetFamily = \Lider\Search\AnalogOfferFiller::detectFamily((string)$__it->name);
                    if ($targetFamily !== '') break;
                }
            }
        }
    }
    $__filler = new \Lider\Search\AnalogOfferFiller($buildReq, $itemToArray, $itemFromArray, $itemCache);
    $allResults = $__filler->fill(
        $allResults,
        $exactKey,
        $normTargetBrand,
        $normTargetArt,
        (string)($targetFamily ?? ''),
        (isset($brandmapMeta) && is_array($brandmapMeta)) ? $brandmapMeta : ((isset($cachedBrandMap) && is_array($cachedBrandMap)) ? $cachedBrandMap : []),
        getSupplierFactory(),
        10,   // max analogs
        45,   // max http
        8.0   // deadline sec
    ...
    // добор не должен ронять поиск
}
// =============================================================================

// =============================================================================
// ANALOG OFFER FILLER (systemic): exact-добор складов по аналогам у ВСЕХ поставщиков
// =============================================================================
try {
    if (!isset($targetFamily) || $targetFamily === '') {
        $targetFamily = '';
    ...
    // debug counter into request-scoped var (manager can see via temporary log)
    $__beforeFill = is_array($allResults) ? count($allResults) : 0;
    ...
    $__afterFill = is_array($allResults) ? count($allResults) : 0;
    @file_put_contents(
        ($_SERVER['DOCUMENT_ROOT'] ?? '/var/www/u3564357/data/www/liderws.ru') . '/upload/logs/analog_fill_' . date('Y-m-d') . '.log',
        date('H:i:s') . " exact={$exactKey} family={$targetFamily} before={$__beforeFill} after={$__afterFill} delta=" . ($__afterFill - $__beforeFill) . " q=" . ($searchNumberRaw ?? '') . " brand=" . ($displayBrand ?? '') . "\n",
        FILE_APPEND
    );
} catch (\Throwable $e) {
    @file_put_contents(
        ($_SERVER['DOCUMENT_ROOT'] ?? '/var/www/u3564357/data/www/liderws.ru') . '/upload/logs/analog_fill_' . date('Y-m-d') . '.log',
        date('H:i:s') . ' ERROR ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine() . "\n",
        FILE_APPEND
    );
}
// =============================================================================
...
// ===== POST-GROUP ANALOG FILL =====
// Добираем недостающих поставщиков прямо в _by_sup карточек аналогов
try {
    if (!isset($targetFamily) || $targetFamily === '') {
        $targetFamily = '';
        if (!empty($targetEntry['description'])) {
    ...
    $__rowBuilder = function (\Lider\Search\SearchResultItem $item) {
        $priceBase = round((float)$item->price, 2);
        if ($priceBase <= 0 && (int)$item->quantity <= 0) {
            return null;
        }
        $priceDisplay = function_exists('getDisplayPrice') ? getDisplayPrice($priceBase) : $priceBase;
        $delivery = function_exists('calcDelivery') ? calcDelivery($item) : ['days' => $item->deliveryDays ?? 0, 'is_approx' => false];
        $stockName = !empty($item->isSched) ? 'Под заказ' : (function_exists('maskWarehouse') ? maskWarehouse($item) : ($item->warehouse ?: '—'));
        $src = (string)$item->source;
        $whKey = $src . '|' . ($item->stockId ?: $item->warehouse) . '|' . $priceDisplay . '|' . ((int)$item->quantity);
        return [
            '__whKey' => $whKey,
    ...
    $__gf = new \Lider\Search\AnalogGroupFiller($buildReq);
    $__res = $__gf->fillMissingSuppliers(
        $groupedItems,
        $exactKey,
        $normTargetBrand,
        $normTargetArt,
        ...
        getSupplierFactory(),
        $__rowBuilder,
        30,  // groups: хватает на single-source аналоги
        90,  // http
        12.0
    );
    $groupedItems = $__res['groups'];
    @file_put_contents(
        ($_SERVER['DOCUMENT_ROOT'] ?? '') . '/upload/logs/analog_fill_' . date('Y-m-d') . '.log',
        date('H:i:s') . ' POSTGROUP exact=' . $exactKey . ' ' . ($__res['log'] ?? '') . "\n",
        FILE_APPEND
    );
    unset($__gf, $__res, $__rowBuilder);
} catch (\Throwable $e) {
    @file_put_contents(
        ($_SERVER['DOCUMENT_ROOT'] ?? '') . '/upload/logs/analog_fill_' . date('Y-m-d') . '.log',
        date('H:i:s') . ' POSTGROUP_ERROR ' . $e->getMessage() . "\n",
        FILE_APPEND
    );
}
// ===== /POST-GROUP ANALOG FILL =====
...
// debug: sources in analog groups
try {
    $__lines = [];
    foreach ($analogGroups as $__k => $__g) {
        $__srcs = [];
        foreach (($__g['warehouses'] ?? []) as $__w) {
            $__s = (string)($__w['source'] ?? '');
            if ($__s !== '') $__srcs[$__s] = true;
        }
        if (count($__srcs) <= 0) continue;
        $blob = mb_strtolower($__k . ' ' . ($__g['brand'] ?? '') . ' ' . ($__g['article'] ?? ''));
        if (str_contains($blob, 'miles') || str_contains($blob, 'afos') || str_contains($blob, 'og154') || str_contains($blob, 'lc219') || count($__srcs) >= 2) {
            $__lines[] = ($__g['brand'] ?? '') . '|' . ($__g['article'] ?? '') . ' srcs=' . implode(',', array_keys($__srcs)) . ' wh=' . count($__g['warehouses'] ?? []);
        }
    }
    if ($__lines) {
        @file_put_contents(($_SERVER['DOCUMENT_ROOT'] ?? '') . '/upload/logs/analog_fill_' . date('Y-m-d') . '.log', date('H:i:s') . " GROUPS exact={$exactKey}\n  " . implode("\n  ", array_slice($__lines, 0, 20)) . "\n", FILE_APPEND);
    }
} catch (\Throwable $e) {}
