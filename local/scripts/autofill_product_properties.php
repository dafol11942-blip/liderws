...
/**
 * Дефис в значении словаря ("5W-40", "75W-90") делаем необязательным при
 * поиске по названию — на практике половина товаров пишет вязкость без
 * дефиса ("5w40", "80W90"). Без этого буквальное сравнение текста тихо
 * не находило совпадение (не путаница со значением, а просто 0 найденных
 * — обнаружилось только когда сравнили с соседними свойствами на тех же
 * товарах, где дефиса в значении не было и всё находилось нормально).
 */
function buildBoundaryRegex(string $needle): string
{
    $parts = array_map(static fn($p) => preg_quote($p, '/'), explode('-', $needle));
    $quoted = implode('-?\s*', $parts);
