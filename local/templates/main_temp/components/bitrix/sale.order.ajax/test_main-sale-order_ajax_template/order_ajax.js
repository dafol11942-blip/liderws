...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		showDeliveryItemsPage: function(page)
		{
			...
			// region_dev = this.regionBlockNode.querySelector('.bx-soa-reference');
			// console.log(region_dev)
			...
			for (k = 0; k < this.deliveryPagination.currentPage.length; k++)
			{
				...
				// console.log(deliveryItemNode)

				// var z = document.createElement('div'); // is a node
				// z.innerHTML = deliveryItemNode;

				// bx-soa-reference сюда епты вставить айтымы, но сначало получи регион доставки
				// region_dev.appendChild(z);
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				title, label, itemNode,currentPaySystem_desc;

			logoNode = BX.create('DIV', {props: {className: 'bx-soa-pp-company-image'}});
			logotype = this.getImageSources(item, 'PSA_LOGOTIP');

			currentPaySystem_desc = BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: item.DESCRIPTION});
			// if (logotype && logotype.src_2x)
			// {
			// 	logoNode.setAttribute('style',
			// 		'background-image: url("' + logotype.src_1x + '");' +
			...
			// }
			label = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-graf-container custom-pay-container'},
				children: [
					BX.create('INPUT', {
						props: {
			...
			itemNode = BX.create('DIV', {
				props: {className: 'pay-System-Items bx-soa-pp-company col-lg-4 col-sm-4 col-xs-6'},
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			if (this.initialized.delivery)
			{
				...
				// var first = BX.firstChild(this.deliveryHiddenBlockNode);
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				propsNode = BX.create('DIV', {props: {className: 'row main_inputs'}});
				selectedDelivery = this.getSelectedDelivery();
		...
		insertDateProperty: function(property, propsItemNode, disabled)
		{
			...
			{
				propContainer = BX.create('DIV', {props: {className: 'soa-property-container lolka'}});
		...
		editPropsComment: function(propsNode)
		{
			...
			input = BX.create('TEXTAREA', {
				props: {
					id: 'orderDescription',
					cols: '4',
					className: 'form-control bx-soa-customer-textarea bx-ios-fix lolka1',
