...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.data_addres_main = parameters.data_addres_main;
			

			if(parameters.data_city_main){
				this.data_city_main = parameters.data_city_main;
			}else{
				this.data_city_main = '0000562524';
			}
			...
			if (this.totalBlockNode)
			{
				...
				// document.querySelector('.cart-sum-delivery')

				this.totalInfoSumDelivery = document.querySelector('.cart-sum-delivery');
			...
			// this.getPersonTypeControl(this.propsBlockNode);
			...
			if (this.params.USER_CONSENT === 'Y')
			{
		   	...
		   	$("input[name='ORDER_PROP_23']").val(this.data_city_main);
		   	$("input[name='ORDER_PROP_4']").val(this.data_city_main);
		    $("#orderDescription").attr("placeholder", "Ваш комментарий...");




			$("input[name='ORDER_PROP_8']").val(this.data_addres_main);
		   	$("input[name='ORDER_PROP_24']").val(this.data_addres_main);


		   	// console.log($("#ID_DELIVERY_ID_26") + 'yesy');
		   	// $("#ID_DELIVERY_ID_24").prop('checked', false);
		   	// $("#ID_DELIVERY_ID_26").prop('checked', true);
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				labelHtml = '<label class="bx-soa-custom-label label-text" for="soa-property-' + parseInt(locationId) + '">'
		...
		getPersonTypeControl: function(node)
		{
			...
			// node = document.getElementById("bx-soa-properties'");

			// var parent = document.querySelector('#bx-soa-properties');
			// 	node = parent;

			// console.log($(node) + 111);

			if(document.querySelector('.form-group-main-type-plata')){

				var parent = document.querySelector('.form-group-main-type-plata').parentNode

				// console.log(parent);
				// var oldChild = element.removeChild(child);
				parent.removeChild(document.querySelector('.form-group-main-type-plata'));
			...
			{
				input = BX.create('DIV', {
					props: {className: 'form-group form-group-main-type-plata'},
					children: [
						BX.create('LABEL', {props: {className: 'bx-soa-custom-label label-text-main'}, html: this.params.MESS_PERSON_TYPE}),
						BX.create('DIV', {props:{className:'type_person'}}),
					]
				});
				...
				node = input;
					...
					// ТУТ МЕНЯЕм местоположение физ и юрид лица
			...
			else if (personTypesCount == 2)
			{
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						node.children[1].appendChild(BX.create('DIV', {props: {className: 'radio-inline'}, children: [label]}));
						delimiter = true;
			...
			else
			{
				for (i in this.result.PERSON_TYPE)
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
						...
						// var parent = document.querySelector('#bx-soa-properties');
						// parent.appendChild(BX.create('INPUT', {props: {type: 'hidden', name: 'PERSON_TYPE', value: this.result.PERSON_TYPE[i].ID}}));
			}
		...
		getProfilesControl: function(node)
		{
			...
			if (profilesLength)
			{
				...
				)
				{
					...
					label = BX.create('LABEL', {props: {className: 'bx-soa-custom-label label-text'}, html: this.params.MESS_SELECT_PROFILE});
		...
		editActivePaySystemBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				paySystemNode = BX.create('DIV', {props: {className: 'bx-soa-pp row wrapper-pay-sistem'}});
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-item-container main-pay-select-container'}}),
				paySystemItemNode, i;
		...
		createPaySystemItem: function(item)
		{
			...
			// }
// MY NEW 
			var currentPaySystem_desc = BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc oplata__text'}, html: item.DESCRIPTION});
			...
			label = BX.create('DIV', {
				...
				// 	}),
					logoNode,
			 		currentPaySystem_desc
			 	]
			});

			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y')
			{
				// title = BX.create('DIV', {props: {className: 'bx-soa-pp-company-smalltitle fw700'}, text: item.NAME});
				title = BX.create('DIV', {
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-lg-4 col-sm-4 col-xs-6 pay-company-select oplata'},
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				deliveryNode = BX.create('DIV', {props: {className: 'bx-soa-pp row wrapper-delivery'}});
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-7 bx-soa-pp-item-container deliveryItemsContainer-main'}}),
				deliveryItemNode, k;
		   ...
		   // 	$("#ID_DELIVERY_ID_26").prop('checked', true);
			...
			// this.regionBlockNode.appendChild(deliveryItemsContainer);
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-5 bx-soa-pp-desc-container editDeliveryInfo'}}),
			...
			// Это не трогать иначе ошибка с NODE
		...
		createDeliveryItem: function(item)
		{
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-lg-4 col-sm-4 col-xs-6 main-container-deliver'},
			...
			if (checked && this.result.LAST_ORDER_DATA.PICK_UP)
				...
				// this.lastSelectedDelivery = 26;


			// (deliveryId == ) && BX.addClass(itemNode, 'bx-selected');
		...
		getSelectedDelivery: function()
		{
			...
			if (deliveryCheckbox)
			{
				...
				for (i in this.result.DELIVERY)
				{
					...
					{
						// console.log(this.result.DELIVERY[i]);
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				propsNode = BX.create('DIV', {props: {className: 'row main-row'}});
				selectedDelivery = this.getSelectedDelivery();
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-customer yes'}}),
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
				...
				if (property.isRequired())
					textHtml += '<div class="isRequired">*</div>';

				label = BX.create('LABEL', {
					attrs: {'for': 'soa-property-' + property.getId()},
					props: {className: 'bx-soa-custom-label label-text-main'},
					html: textHtml
				});
		...
		insertDateProperty: function(property, propsItemNode, disabled)
		{
			...
			{
				propContainer = BX.create('DIV', {props: {className: 'soa-property-container main_input-container'}});
		...
		insertFileProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				{
					propContainer = BX.create('DIV', {props: {className: 'soa-property-container main'}});
					property.appendTo(propContainer);
					propsItemNode.appendChild(propContainer);
					this.alterProperty(property.getSettings(), propContainer);
				}
		...
		insertStringProperty: function(property, propsItemNode, disabled)
		{
			...
			{
				propContainer = BX.create('DIV', {props: {className: 'soa-property-container main1'}});
				property.appendTo(propContainer);
				propsItemNode.appendChild(propContainer);
				this.alterProperty(property.getSettings(), propContainer);
				this.bindValidation(property.getId(), propContainer);
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'col-sm-12 comment-block'}});
			label = BX.create('LABEL', {
				attrs: {for: 'orderDescription'},
				props: {className: 'bx-soa-customer-label label-text-main'},
		...
		editTotalBlock: function()
		{
			...
			// Это всего сумма заказов 

			// document.querySelector('.cart-sum').appendChild(this.createTotalUnit(BX.message('SOA_SUM_SUMMARY'), priceHtml, params));
			...
			else
			{
				...
				{
					deliveryValue = total.DELIVERY_PRICE;
				}
			...
			if (this.result.DELIVERY.length)
			{
				...
				document.querySelector('.cart-sum-delivery').innerHTML = deliveryValue;
			...
			{	
				...
				document.querySelector('.cart-total-sum').innerHTML = total.ORDER_TOTAL_PRICE;
				document.querySelector('.cart-total-sum-2').innerHTML = total.ORDER_TOTAL_PRICE;
		...
		createTotalSum: function(name, value, params)
		{
