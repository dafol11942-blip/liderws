...
/**
 * Делает то же, что FullSearchLauncher, но ПЛЮС:
 * — все найденные результаты сохраняет в b_supplier_stock (кэш)
 * — возвращает результаты, помечая source_cache_hit / source_live
 */
class CachingFullSearchLauncher
{
    private SupplierFactory $factory;
    private InstantSearcher $cache;

    public function __construct(SupplierFactory $factory, InstantSearcher $cache)
    {
    ...
    public function launch(
        ...
        $bfs = fn($sup, $e, $fb) => ($e && !empty($e['brands'][$sup->getCode()]))
            ? (string)$e['brands'][$sup->getCode()]
            : BrandNormalizer::displayBrand($fb);
        $afs = fn($sup, $e, $fa) => ($e && !empty($e['articles'][$sup->getCode()]))
            ? (string)$e['articles'][$sup->getCode()]
            : $fa;

        // === ФАЗА 1: exact + nobrand ===
        $p1r = []; $p1m = [];
        foreach ($suppliers as $sup) {
            $code = $sup->getCode();
            $sb = $bfs($sup, $targetEntry, $brand);
            $sa = $afs($sup, $targetEntry, $article);
            ...
            if ($req = $sup->buildSearchRequest($sb, $sa, false)) {
                ...
                $req['_priority'] = 0;
                $p1r[] = $req;
                $p1m[$k] = ['sup' => $sup, 'brand' => $sb, 'article' => $sa];
            }
            if ($req = $sup->buildSearchRequest('', $sa, false)) {
                ...
                $req['_priority'] = 1;
                $p1r[] = $req;
                $p1m[$k] = ['sup' => $sup, 'brand' => '', 'article' => $sa, 'noBrand' => true];
            }
        }

        $e1 = new MultiCurlExecutor();
        $r1 = $e1->executeAll($p1r, $deadline * 0.5);
                ...
                foreach ($items as $item) {
                    ...
                    if (!$isExact) {
                        if (!isset($analogMap[$itemNormArt])) {
                            $analogMap[$itemNormArt] = [
                                'brands' => [$gk => $item->brand],
                                'articles' => [$gk => $item->article],
                                'sources' => [$src => true],
                                'example' => $item
                            ];
                        } else {
                            $analogMap[$itemNormArt]['sources'][$src] = true;
                            if (!isset($analogMap[$itemNormArt]['brands'][$gk]))
                                $analogMap[$itemNormArt]['brands'][$gk] = $item->brand;
        ...
        // Сохраняем фазу 1 в кэш
        if (!empty($toCache)) {
            $saved = $this->cache->saveResults($toCache);
            $this->log("P1 cached: {$saved}");
        }

        // === ФАЗА 2: добираем аналоги у недостающих поставщиков ===
        ...
        foreach ($analogMap as $normArt => $am) {
            ...
            $bmFound = null;
            foreach ($am['brands'] as $gk => $b) {
                if (isset($brandMap[$gk])) {
                    $bmFound = $brandMap[$gk];
                    break;
                }
            }
            foreach ($missing as $supCode) {
                ...
                $ta = $fa;
                if ($bmFound) {
                    $tb = !empty($bmFound['brands'][$supCode])
                        ? (string)$bmFound['brands'][$supCode] : $tb;
                    $ta = !empty($bmFound['articles'][$supCode])
                        ? (string)$bmFound['articles'][$supCode] : $ta;
                }
                ...
                $req['_priority'] = 3;
                $p2r[] = $req;
                $p2m[$k2] = ['sup' => $sup, 'normArt' => $normArt, 'brand' => $tb, 'article' => $ta];
            }
        }

        if (!empty($p2r)) {
            $p2Deadline = max(10.0, $deadline * 0.6);
            $this->log("P2: " . count($p2r) . " requests for " .
                count(array_unique(array_column($p2m, 'normArt'))) . " analogs");
            $e2 = new MultiCurlExecutor();
            $r2 = $e2->executeAll($p2r, $p2Deadline);
            $added = 0;
            $p2Cache = [];
            ...
            if (!empty($p2Cache)) {
                $this->cache->saveResults($p2Cache);
            }
        ...
        usort($results, fn($a, $b) =>
            (!$a->isSched && $b->isSched) ? -1 :
            (($a->isSched && !$b->isSched) ? 1 : $a->price <=> $b->price)
        );
        return $results;
    }
    ...
    {
        @file_put_contents(
            '/var/www/u3564357/data/www/liderws.ru/upload/logs/caching_search_' . date('Y-m-d') . '.log',
            '[' . date('H:i:s') . '] ' . $msg . "\n",
            FILE_APPEND
        );
    }
