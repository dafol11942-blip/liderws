...
class BergConnector implements SupplierInterface, SupplierOrderable, SupplierOrderStatusProvider
{
    ...
    public function fetchOrderStatusByReference(string $reference): array
    {
        ...
        return [[
            ...
            'stage'           => $this->normalizeStage($stateText, $stateType, isset($state['id']) ? (int)$state['id'] : null),
    ...
    // Полный официальный справочник /references/states (снят вживую 2026-09-08,
    // id => [название, type]):
    //   1 Обнулен(2)  2 Отгружен(2)  3 Снят с резерва(2)
    //   4 Зарезервирован: нет в наличии(0)  5 Зарезервирован: есть в наличии(0)
    //   6 Ожидает отправки заказа поставщику(0)  7 Готов к отгрузке(2)
    //   8 Заказ подтвержден(0)  9 Ожидает подтверждения(0)  10 Товар в сборке(0)
    //   11 На центральном складе(0)  12 Зарезервирован на ЦС: есть в наличии(0)
    //   13 Зарезервирован на ЦС: нет в наличии(0)  14 Собирается на ЦС(0)
    //   15 На складе филиала(0)  296 Создан заказ(1)
    //   297 Не зарезервирован. Свяжитесь с менеджером(5 — нестандартный type, не
    //   входит в задокументированные 0/1/2, но явно требует внимания менеджера)
    //   298 Товар в пути(0)  657 Зарезервирован МСК2: есть в наличии(0)
    //   660 Перемещение с МСК2 на ЦС(0)  663 Задержка поставки(0)
    //
    // type=2 ("присваивается по завершению обработки") НЕ означает автоматически
    // успех — id=1 и id=3 тоже type=2, но это отказ (обнулили/сняли с резерва,
    // в UI Берга обе показаны как "Отменён"). Поэтому классифицируем в первую
    // очередь по конкретному id из этого справочника, а не по одному type —
    // фразы и type=2-фолбэк остаются только подстраховкой для id, которых
    // Берг ещё не присылал (справочник мог измениться/пополниться).
    private const REFUSED_STATE_IDS = [1, 3, 297];
    private const READY_STATE_IDS   = [2, 7];

    private const REFUSED_PHRASES    = ['отказ', 'отменен', 'отменён', 'возврат', 'не может быть поставлен', 'не будет поставлен', 'снят с резерва', 'обнулен', 'свяжитесь с менеджером'];
    private const READY_PHRASES      = ['получен', 'выдан', 'доставлен клиенту', 'закрыт', 'завершен', 'завершён', 'отгружен', 'готов к отгрузке'];
    private const IN_TRANSIT_PHRASES = ['отгруж', 'передан', 'в пути', 'собран', 'складе', 'задержка поставки'];

    private function normalizeStage(?string $stateText, ?int $stateType, ?int $stateId): string
    {
        if ($stateId !== null) {
            if (in_array($stateId, self::REFUSED_STATE_IDS, true)) return 'refused';
            if (in_array($stateId, self::READY_STATE_IDS, true)) return 'ready';
        }
