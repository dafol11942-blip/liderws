...
// МИГРАЦИОННАЯ ФУНКЦИЯ, новых вызовов не добавлять. Раньше при клике «Перейти
// к оформлению» неотмеченные чекбоксом позиции физически удалялись из корзины
// со снимком в PHP-сессии (action=stashUnselected, убран — см. ajax/basket.php)
// и возвращались обратно сюда при следующем заходе на /cart/. Сейчас
// исключение неотмеченных позиций из заказа делается фильтром по CART_SELECTED
// без удаления чего-либо из корзины (см. order_create_handler.php), новые
// снимки в $_SESSION не создаются — эта функция лишь доедает то, что могло
// остаться в сессиях, открытых до перехода на новую схему.
...
function getSupplierFactory(): \Lider\Supplier\SupplierFactory
{
    ...
    if ($factory === null) {
        ...
        $factory->register(new \Lider\Supplier\RosskoConnector([
            ...
            // Второй адрес (Баки Урманче, KEY1 c90231f7.../KEY2 67a36697...,
            // address_id=131532, requisite_id=39576) ВРЕМЕННО ОТКЛЮЧЁН —
            // ACCOUNTS_BY_WAREHOUSE намеренно не задан, все заказы уходят на
            // дефолтный аккаунт (Нефтяников), независимо от выбранного в
            // форме склада. Причина: заказы №205/206/208 (оба аккаунта,
            // разные артикулы, склад Набережные Челны) стабильно отклонялись
            // GetCheckout с "Нет в наличии" при подтверждённом реальном
            // наличии (сайт Росско и GetSearch показывают товар доступным,
            // заказ через их сайт проходит) — воспроизведено вживую 2026-09-14,
            // похоже на баг конкретного stock-кода (HST46343836) в их API,
            // не связанный с выбором аккаунта/склада (не помогло даже
            // возвращение на дефолтный ключ, см. заказ №207). Включить обратно,
            // когда Росско подтвердят исправление на своей стороне — код
            // поддержки accountsByWarehouse в RosskoConnector менять не нужно,
            // только вернуть этот блок конфигурации.
        ]));
        ...
        $factory->register(new \Lider\Supplier\ArmtekConnector([
            ...
            'KUNZA' => '48022996',
            'VBELN' => '40359920',
            // Второй кабинет — Баки Урманче — отдельный логин lider2-16@bk.ru
            // (по аналогии с ПартКомом/Автопитером), но НЕ отдельный клиент:
            // проверено через getUserVkorgList/getUserInfo под этим логином —
            // тот же VKORG=4000, тот же покупатель KUNRG=43039417 (со слов
            // поставщика значились VKORG=4220/КУНРГ=40039944 — оба оказались
            // неверны, ровно та же ошибка "Не установлена сбытовая
            // организация", что и в первый раз; 40039944 — это KUNAG верхнего
            // уровня, а не KUNRG). Меняется только KUNZA (второй адрес в
            // ZA_TAB того же KUNRG — "...Елабуга, ул. Урманче, 17А"). VBELN —
            // тот же договор, второй VBELN из DOGOVOR_TAB (40359921) для этого
            // адреса не подошёл ("По запросу ничего не найдено" на живом
            // createTestOrder). Оба — LOGIN/PASSWORD и KUNZA — проверены живым
            // createTestOrder под lider2-16@bk.ru (ERROR:0).
            'ACCOUNTS_BY_WAREHOUSE' => [
                'baki_urmanche' => [
                    'LOGIN'    => 'lider2-16@bk.ru',
                    'PASSWORD' => 'LidGates16',
                    'KUNZA'    => '48104498',
                ],
            ],
