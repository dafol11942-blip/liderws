<?php
/**
 * analog_search.php (v4 — Этап 17.4)
 * Архитектура: b_cross_index → b_supplier_stock → JSON
 * Только Москворечье (прайс-лист). Без API-запросов к поставщикам.
 * Холодный: UMAPI (1с) → ответ <2с.
 * Горячий: чистый SQL <200мс.
 */
...
const SUPPLIER_NAME = 'Москворечье';
...
function maskWarehouse(string $realName): string {
    if ($realName === '' || $realName === '—') return '—';
    if (isManager()) return SUPPLIER_NAME . ': ' . $realName;
    // Транслит из MoskvorechieConnector
    static $map = [
    ...
    $lower = mb_strtolower(trim($realName));
    $translit = '';
...
try {
    ...
    // Валидация
    $expectedToken = md5($q . '|' . $brand . '|' . $number . '|analog_v4');
    ...
    $cacheKey = md5(implode('|', [$q, $brand, $number, $filterPriceMin, $filterPriceMax, $filterBrand, isManager() ? 'mgr' : 'usr', 'v4']));
    ...
    // ─── 5. b_supplier_stock (мгновенно, только Москворечье) ──
    ...
    $stockRows = $db->query(
        "SELECT supplier_code, article, brand, name, price, quantity,
         ...
         WHERE supplier_code = '" . SUPPLIER_CODE . "'
           AND article_normalized IN ($inClause)
           AND is_active = 1
...
<?php foreach ($group['warehouses'] as $wh):
    ...
    if (!isManager()) {
        $stockDisplay = maskWarehouse($stockDisplay);
    } else {
        if ($stockDisplay !== 'Под заказ' && $stockDisplay !== '—') {
            $stockDisplay = SUPPLIER_NAME . ': ' . $stockDisplay;
        }
    }
    ...
    $sourceTag = isManager() ? '<span class="source-tag source-tag--moskvorechie">' . SUPPLIER_NAME . '</span>' : '';
