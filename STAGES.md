...
Итоги Этапа 11 — Подключение прайс-листов
...
1. Прайс-листы есть у 5 из 10 поставщиков


Поставщик	Источник	Формат
Москворечье	ABCP выгрузка	CSV (windows-1251)
ПартКом	ABCP выгрузка	CSV
Авторусь	ABCP выгрузка	CSV
Rossko	свой	CSV
Авто-Евро	XLSX/CSV	Производитель^НомерПроизводителя



Остальные (Berg, Ixora, Tatparts, Autopiter, Shatem) — только live API.



2. Прайс-листы не решают проблему аналогов



CSV/Excel содержат только: бренд, артикул, цену, остаток — но не содержат кросс-номера (какой артикул является аналогом какого). Без отдельного источника аналогов переход на прайс-листы означает потерю блока «Аналоги».



3. Найден внешний источник кроссов: UMAPI



GET /crosses/by_code?code=W81180 — возвращает все аналоги по артикулу за ~100-300 мс (против 3-6 сек у текущего BrandMap через 8 API-запросов). API-ключ есть.



4. Проблема устаревания цен решена архитектурно



Трёхуровневая модель: каталог (крон каждые 2-4ч) → фоновая live-проверка при lazy-loader → обязательный live-запрос при заказе.
...
Прайс-листы — это оптимизация скорости (20-30 сек → < 1 сек). Но первичная задача — корректность аналогов. Если сначала внедрить UMAPI, то:




Аналоги станут полнее и быстрее (1 запрос вместо 8)


Архитектура станет чище (единый источник кроссов)


Прайс-листы будут накладываться поверх уже работающей схемы, а не ломать её
...
Этап 11 (этот):     Анализ прайс-листов → ОТЛОЖЕН ✅
Этап 12 (следующий): UMAPI вместо BrandMap — пересборка аналогов
Этап 13 (будущий):   Прайс-листы — когда архитектура готова
